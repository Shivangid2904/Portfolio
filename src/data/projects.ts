...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      "A local-first RAG system for asking questions over PDF documents — no external API calls, no data leaving your machine. It uses Sentence Transformers for embeddings, FAISS for hybrid retrieval, and Ollama for local LLM inference. The Streamlit interface makes it easy to drop in a document and start asking questions.",
  ...
  },
  {
    ...
    description:
      "A scientific ML project using NASA Exoplanet Archive data. The dataset has severe class imbalance — confirmed habitable candidates are rare — so I built a Physics-Informed Proxy Model to guide the learning, and benchmarked Random Forest and XGBoost with stratified cross-validation. SHAP explanations make the model's reasoning visible rather than treating it as a black box.",
